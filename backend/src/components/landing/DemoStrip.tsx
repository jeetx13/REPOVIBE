...
export function DemoStrip() {
  ...
  useEffect(() => {
    if (!inView) return;
    let alive = true;
    fetchDemoAnalysis((DEMO_REPOS[active] ?? DEMO_REPOS[0]).key).then((a) => {
      if (!alive) return;
        ...
        <div className="mt-10 grid lg:grid-cols-[1fr_1.4fr] gap-6">
          ...
          >
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-xs font-mono text-charcoal-700/50">{(DEMO_REPOS[active] ?? DEMO_REPOS[0]).name}</span>
            </div>
